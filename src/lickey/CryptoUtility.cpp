...
namespace lickey
{
	bool Encrypt(const std::string& data, const size_t datalen, std::string key, std::string iv, std::string& dest,
	             size_t& destlen)
	{
	...
	bool Decrypt(const std::string& data, const size_t datalen, std::string key, std::string iv, std::string& dest,
	             size_t& destlen)
	{
	...
	bool MD5(std::string data, size_t datalen, std::string& hash)
	{
	...
	bool SHA256(std::string& data, size_t datalen, std::string& hash)
	{
		hash = make_hash(hash_t::sha256, data);
		return false;
	}

	void EncodeBase64(std::string& data, std::string& str)
	{
		str = to_base64(data);
	}

	void DecodeBase64(std::string& str, std::string& data, int& datalen)
	{
